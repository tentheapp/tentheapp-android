...
public class EditProfileFragment extends BaseFragment implements View.OnClickListener, ImageChooserListener,
    ...
    protected void initViews(View view) {
        ...
        RelativeLayout edit_profile_layout = (RelativeLayout) findViewById(R.id.edit_profile_layout);
        ...
        mEditProfileCheckBox.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == ChooserType.REQUEST_CAPTURE_PICTURE ||
                ...
                Glide.with(EditNewProfileFragment.this)
                        .load(editedImageUri)
                    ...
                    if (response.body().getSuccess().equals("true")) {
                        ...
                        /*Thread thread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                File dfs = Glide.getPhotoCacheDir(getBaseActivity());
                                if (dfs != null) {
                                    String[] list = dfs.list();
                                    for (int d = 0; d < list.length; d++) {
                                        String name = list[d];
                                        Log.d("---", "------" + name);
                                        File fff = new File(dfs.getPath(), name);
                                        if (fff.exists()) {
                                            boolean isDdd = fff.delete();
                                            Log.d("---", "------" + isDdd);
                                        }
                                    }
                                }
