...
public class ProfileActivity extends BaseActivity implements View.OnClickListener, ProfileApi.ProfileCallbacks,
        View.OnLongClickListener, ProfilePresenter.UsersProfileCallback, PopupMenu.OnMenuItemClickListener, GetActivePostsPresenter.ActivePostCallback {
...
/*
    ...
    public void setAdapter(RecyclerView recyclerView, ArrayList<?> posts) {
        ...
        if (posts != null && posts.size() > 0) {
            ...
            adapter = new NewsPostAdapter(this, posts, linearLayoutManager);
...
/*mUserId = data.getUser().getIdUser();
                ...
                Glide.with(mContext)
                        .load(HttpUtils.getProfileImageURL(mUserName))
                        .error(mContext.getResources().getDrawable(R.drawable.myprofilelarge))
        ...
        if (response != null) {
            ...
            if (body != null) {
                ...
                if (posts == null || posts.size() == 0) {
                    ...
                    mPostsProgressBar.setVisibility(View.INVISIBLE);
                    Toast.makeText(mContext, "No Active post.", Toast.LENGTH_LONG).show();
                }
            } else {
                mPostsProgressBar.setVisibility(View.INVISIBLE);
                Toast.makeText(mContext, "No Active post.", Toast.LENGTH_LONG).show();
