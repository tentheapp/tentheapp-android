...
//        }
        if (data.getType() != null) {
            ...
            String notiId = data.getIdNotification();
            String noti_msg = data.getMessage();
            String taggedName = "";
            if (notiId.equals("0")){
                holder.typeIcon.setImageResource(R.drawable.grouping_icon);
                if (noti_type.contains("COMMENT")) {
                    noti_msg =data.getNotificationIds().length+" comments";
                } else if (noti_type.contains("LIKE")) {
                    noti_msg = data.getNotificationIds().length+" timed";
                }
            }else {
                if (noti_type.contains("COMMENT")) {
                ...
                if (data.getUser()!=null){
                    taggedName = data.getUser().getUsername();
                    if (taggedName!=null && taggedName.length()>0){
                        if (noti_type.contains("COMMENT")) {
                            noti_msg = "@"+taggedName +" commented your post";
                        }  else if (noti_type.contains("LIKE")) {
                            noti_msg = "@"+taggedName +" timed your post";
                        }else {
                            String[] messArray = noti_msg.split(" ");
                            if (messArray.length>0){
                                taggedName = messArray[0];
                            }
                            noti_msg = "@"+ noti_msg;
                        }
                    }
            ...
            if (noti_msg.contains("@")) {
                final String finalTaggedName = taggedName;
                SpannableString styledString = new SpannableString(noti_msg);
                ClickableSpan clickableSpan = new ClickableSpan() {
                    ...
                    public void onClick(View widget) {
                        ...
                        bundle.putString("username", finalTaggedName);
                        notificationFragment.replaceChildFragment(R.id.notification_frame_layout, new NewProfileFragment(),
                                bundle);
                    }
                    ...
                    public void updateDrawState(TextPaint ds) {
                        ...
                        ds.setColor(mActivity.getResources().getColor(R.color.tenBlue));
                ...
                if (taggedName != null && taggedName.length() > 0) {
                    ...
                    if (startIndex != -1 && endIndex <= styledString.length()) {
                        ...
                        styledString.setSpan(clickableSpan, startIndex, endIndex, 0);
                        holder.notiMsg.setText(styledString);
                    }
                }
            }else {
                ...
                switch (view.getId()) {
                    case R.id.user_name:
                        if (notificationList.get(clickedPosition).getUser() != null)
                            notificationFragment.moveToProfile(notificationList.get(clickedPosition).getUser().getUsername(), notificationList.get(clickedPosition).getIdNotification());
                        break;
                    case R.id.user_img:
                        if (notificationList.get(clickedPosition).getUser() != null)
                            notificationFragment.moveToProfile(notificationList.get(clickedPosition).getUser().getUsername(), notificationList.get(clickedPosition).getIdNotification());
