...
public class FollowersAdapter extends RecyclerView.Adapter<FollowersAdapter.ViewHolder> implements Core {

    private SharedPrefsHelper sharedPref;
    private ArrayList<FollowingResponse.Users> mList;
    private BaseActivity mActivity;
    private BaseFragment mBaseFragment;

    public FollowersAdapter(BaseActivity context, ArrayList<?> mList) {
        this.mList = (ArrayList<FollowingResponse.Users>) mList;
        this.mActivity = context;
        sharedPref = new SharedPrefsHelper(context);
    }

    public FollowersAdapter(BaseFragment context, ArrayList<?> mList) {
        this.mList = (ArrayList<FollowingResponse.Users>) mList;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View itemLayoutView = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.row_followers, null);
        // create ViewHolder
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        Glide.with(mActivity)
                ...
                .bitmapTransform(new RoundedCornersTransformation(mActivity, 15, 12))
        ...
        if (position == mList.size() - 1) {
            if (mBaseFragment != null) {
                if (mBaseFragment instanceof FollowersFragment) {
                    FollowersFragment followersFragment = (FollowersFragment) mBaseFragment;
                    followersFragment.loadPaginationData(mList.size());
                }
            }else {
                mActivity.refreshList(mList.size());
            }
        }
            ...
            switch (view.getId()) {
                case R.id.rl_follow:
                    if (mBaseFragment == null) {
                        ...
                        mBaseFragment.replaceChildFragment(R.id.frame_layout_followers, new NewProfileFragment(), bundle);
                    }
                    break;
